...
function SkillCell({
  ...
  return (
    <motion.div
      ...
      className="group relative flex flex-col p-3 rounded-xl transition-all duration-200"
      style={{
        background: bg,
        border: `1px solid ${accent}22`,
      }}
    >
      <span className="text-xs font-medium leading-tight" style={{ color: "hsl(var(--text-heading))" }}>
        {skill}
      </span>
      <ProficiencyDots level={level} accent={accent} />
      {count > 0 && (
        <span
          className="absolute top-1.5 right-2 text-[9px] font-mono"
...
function pseudoLevel(skill: string, max = 5): number {
              ...
              <LayerReveal key={group.category} delay={groupIdx * 60}>
                <div>
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    <span
                      className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-[10px] font-mono font-semibold tracking-wider"
                      style={{ background: color.bg, border: `1px solid ${color.accent}33`, color: color.accent }}
                    >
                      <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: color.accent }}/>
                      {group.category}
                    </span>
                    <span className="text-[10px] font-mono" style={{ color: "hsl(var(--text-dim))" }}>
